...
_Bool LCD_Init ( LCD display ) {
    ...
    LCD_Write(0x03); _delay((unsigned long)((5)*(4000000/4000.0)));
    LCD_Write(0x03); _delay((unsigned long)((16)*(4000000/4000.0)));
    LCD_Write(0x03);
    LCD_Write(0x02);

    do { LCD_Write( (0x28 & 0xF0) >> 4 ); LCD_Write( 0x28 & 0x0F); } while ( 0 );
    do { LCD_Write( (0x06 & 0xF0) >> 4 ); LCD_Write( 0x06 & 0x0F); } while ( 0 );
    do { unsigned char cmd = 0x08; if ( 1 ) cmd |= 1 << 2; if ( 0 ) cmd |= 1 << 1; if ( 0 ) cmd |= 1; do { LCD_Write( (cmd & 0xF0) >> 4 ); LCD_Write( cmd & 0x0F); } while ( 0 ); } while ( 0 );
...
void LCD_cg_ram_addr(uint8_t addr) {
    do { LCD_Write( (0x40 | (addr & 0x3F) & 0xF0) >> 4 ); LCD_Write( 0x40 | (addr & 0x3F) & 0x0F); } while ( 0 );
