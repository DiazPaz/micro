...
int password[4] = {1, 0, 0, 0};
int userInput[4] = {0, 0, 0, 0};
unsigned char tries = 3;


unsigned char prev_RB1 = 1, prev_RB2 = 1, prev_RB3 = 1, prev_RB4 = 1;
...
void printPass() {
    for(int i = 0; i < 25; i++) {
        PORTC = 0b11111110; PORTD = PASS[0]; _delay((unsigned long)((1)*(4000000/4000.0)));
        PORTC = 0b11111101; PORTD = PASS[1]; _delay((unsigned long)((1)*(4000000/4000.0)));
        PORTC = 0b11111011; PORTD = PASS[2]; _delay((unsigned long)((1)*(4000000/4000.0)));
        PORTC = 0b11110111; PORTD = PASS[3]; _delay((unsigned long)((1)*(4000000/4000.0)));
        PORTBbits.RB5 = 1;
    }
    clearLEDs();
}
...
void printFail() {
    for(int i = 0; i < 25; i++) {
        PORTC = 0b11111110; PORTD = FAIL[0]; _delay((unsigned long)((1)*(4000000/4000.0)));
        PORTC = 0b11111101; PORTD = FAIL[1]; _delay((unsigned long)((1)*(4000000/4000.0)));
        PORTC = 0b11111011; PORTD = FAIL[2]; _delay((unsigned long)((1)*(4000000/4000.0)));
        PORTC = 0b11110111; PORTD = FAIL[3]; _delay((unsigned long)((1)*(4000000/4000.0)));
        if(tries > 0) PORTBbits.RB7 = 1;
        else PORTBbits.RB6 = 1;
    }
    clearLEDs();
}
...
void main(void) {
    ...
    while (1) {
        ...
        if (!PORTBbits.RB1 && prev_RB1 && pos < 4) {
            userInput[pos++] = 1;
            prev_RB1 = 0;
        }
        if (PORTBbits.RB1) prev_RB1 = 1;

        if (!PORTBbits.RB2 && prev_RB2 && pos < 4) {
            userInput[pos++] = 0;
            prev_RB2 = 0;
        }
        if (PORTBbits.RB2) prev_RB2 = 1;

        if (!PORTBbits.RB3 && prev_RB3 && pos < 4) {
            userInput[pos++] = 1;
            prev_RB3 = 0;
        }
        if (PORTBbits.RB3) prev_RB3 = 1;

        if (!PORTBbits.RB4 && prev_RB4 && pos < 4) {
            userInput[pos++] = 0;
            prev_RB4 = 0;
        }
        if (PORTBbits.RB4) prev_RB4 = 1;


        for (int i = 0; i < 20; i++) {
            PORTC = 0b11111110;
            PORTD = digits[tries - 1];
